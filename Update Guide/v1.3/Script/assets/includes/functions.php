...
function createNotification($data = []) {
    ...
    $senderEmailNotification = $db->arrayBuilder()->where('id', $data['notifier_id'] )->getOne(T_USERS,array('id','username','name','email','avatar','email_on_follow_user','email_on_liked_track','email_on_liked_comment','email_on_artist_status_changed','email_on_receipt_status_changed','email_on_new_track','email_on_reviewed_track'));
    $userEmailNotification = $db->arrayBuilder()->where('id', $data['recipient_id'])->getOne(T_USERS,array('id','username','name','email','avatar','email_on_follow_user','email_on_liked_track','email_on_liked_comment','email_on_artist_status_changed','email_on_receipt_status_changed','email_on_new_track','email_on_reviewed_track'));
    $u = $music->user;
...
function deleteUser($user_id = 0) {
    ...
    $getUserAds = $db->where('user_id', $user_id)->get(T_ADS);
...
function get_user_ads($placement = 1) {
    global $db,$music;
    ...
    $ad = $db->rawQuery("select * FROM ".T_USR_ADS." where status = 1 AND placement = {$placement} {$execlude_text} AND ( day_limit > day_spend OR day_limit = 0 ) ORDER BY RAND() LIMIT 1");
...
function ImportFormSoundCloud($trackUrl){
    ...
    $imported = $db->where('src', 'SOUNDCLOUD:'.trim($data['id']))->getOne(T_SONGS);
    if (!empty($imported)) {
        ...
        $Audio = "upload/audio/" . date('Y') . '/' . date('m') . '/' . generateKey() . '_' . date('d') . '_' . md5(time()) . "_audio.mp3";
        file_put_contents($Audio, file_get_contents($sound['http_mp3_128_url']));

        if($music->config->soundcloud_go == 'off') {
            $getID3 = new getID3;
