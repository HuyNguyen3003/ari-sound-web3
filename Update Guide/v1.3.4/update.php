<?php
if (file_exists('./assets/init.php')) {
    require_once('./assets/init.php');
} else {
    die('Please put this file in the home directory !');
}
function PT_UpdateLangs($lang, $key, $value) {
    global $sqlConnect;
    $update_query         = "UPDATE langs SET `{lang}` = '{lang_text}' WHERE `lang_key` = '{lang_key}'";
    $update_replace_array = array(
        "{lang}",
        "{lang_text}",
        "{lang_key}"
    );
    return str_replace($update_replace_array, array(
        $lang,
        mysqli_real_escape_string($sqlConnect, $value),
        $key
    ), $update_query);
}
$updated = false;
if (!empty($_GET['updated'])) {
    $updated = true;
}
if (!empty($_POST['query'])) {
    $query = mysqli_query($mysqli, base64_decode($_POST['query']));
    if ($query) {
        $data['status'] = 200;
    } else {
        $data['status'] = 400;
        $data['error']  = mysqli_error($mysqli);
    }
    header("Content-type: application/json");
    echo json_encode($data);
    exit();
}
if (!empty($_POST['update_langs'])) {
    $data  = array();
    $query = mysqli_query($sqlConnect, "SHOW COLUMNS FROM `langs`");
    while ($fetched_data = mysqli_fetch_assoc($query)) {
        $data[] = $fetched_data['Field'];
    }
    unset($data[0]);
    unset($data[1]);
    unset($data[2]);
    unset($data[3]);
    $lang_update_queries = array();
    foreach ($data as $key => $value) {
        $value = ($value);
        if ($value == 'arabic') {
            $lang_update_queries[] = PT_UpdateLangs($value, 'first_name', 'الاسم الأول');
            $lang_update_queries[] = PT_UpdateLangs($value, 'too_many_login_attempts_please_try_again_later', 'الكثير من محاولات تسجيل الدخول، يرجى المحاولة مرة أخرى لاحقا.');
            $lang_update_queries[] = PT_UpdateLangs($value, 'please_choose_which_information_you_would_like_to_download', 'يرجى اختيار المعلومات التي تريد تنزيلها.');
            $lang_update_queries[] = PT_UpdateLangs($value, 'my_information', 'معلوماتي');
            $lang_update_queries[] = PT_UpdateLangs($value, 'generate_file', 'توليد الملف');
            $lang_update_queries[] = PT_UpdateLangs($value, 'your_file_is_ready_to_download_', 'ملفك جاهز للتنزيل!');
            $lang_update_queries[] = PT_UpdateLangs($value, 'avatar', 'الصورة الرمزية');
            $lang_update_queries[] = PT_UpdateLangs($value, 'cover', 'غطاء، يغطي');
            $lang_update_queries[] = PT_UpdateLangs($value, 'hall_of_fame', 'قاعة الشهرة');
            $lang_update_queries[] = PT_UpdateLangs($value, 'enabled', 'مكن');
            $lang_update_queries[] = PT_UpdateLangs($value, 'disabled', 'معاق');
            $lang_update_queries[] = PT_UpdateLangs($value, 'donate_button', 'تبرع زر');
            $lang_update_queries[] = PT_UpdateLangs($value, 'to_enable_donate_button_please_put_your', 'لتمكين زر التبرع يرجى وضع');
            $lang_update_queries[] = PT_UpdateLangs($value, 'donate', 'يتبرع');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tag_artists', 'فنانين العلامة');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tag_other_artists_to_show_they_performed_together', 'علامة الفنانين الآخرين لإظهار أنها تؤدي معا');
            $lang_update_queries[] = PT_UpdateLangs($value, 'no_request_found', 'لم يتم العثور على طلب');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tagged_you', '');
            $lang_update_queries[] = PT_UpdateLangs($value, 'approve', 'يوافق');
            $lang_update_queries[] = PT_UpdateLangs($value, 'decline', 'يتناقص');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tagged_artists', 'الفنانين الموسومة');
        } else if ($value == 'dutch') {
            $lang_update_queries[] = PT_UpdateLangs($value, 'first_name', 'Voornaam');
            $lang_update_queries[] = PT_UpdateLangs($value, 'too_many_login_attempts_please_try_again_later', 'Te veel inlogpogingen Probeer het later opnieuw.');
            $lang_update_queries[] = PT_UpdateLangs($value, 'please_choose_which_information_you_would_like_to_download', 'Kies welke informatie u wilt downloaden.');
            $lang_update_queries[] = PT_UpdateLangs($value, 'my_information', 'Mijn informatie');
            $lang_update_queries[] = PT_UpdateLangs($value, 'generate_file', 'Het genereren van het bestand');
            $lang_update_queries[] = PT_UpdateLangs($value, 'your_file_is_ready_to_download_', 'Uw bestand is klaar om te downloaden!');
            $lang_update_queries[] = PT_UpdateLangs($value, 'avatar', 'Avatar');
            $lang_update_queries[] = PT_UpdateLangs($value, 'cover', 'Hoes');
            $lang_update_queries[] = PT_UpdateLangs($value, 'hall_of_fame', 'Hall of fame');
            $lang_update_queries[] = PT_UpdateLangs($value, 'enabled', 'Ingeschakeld');
            $lang_update_queries[] = PT_UpdateLangs($value, 'disabled', 'Gehandicapt');
            $lang_update_queries[] = PT_UpdateLangs($value, 'donate_button', 'Doneer knop');
            $lang_update_queries[] = PT_UpdateLangs($value, 'to_enable_donate_button_please_put_your', 'Om Donate-knop in te schakelen, zet u dan uw');
            $lang_update_queries[] = PT_UpdateLangs($value, 'donate', 'Doneren');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tag_artists', 'Tag artiesten');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tag_other_artists_to_show_they_performed_together', 'Tag andere artiesten om te laten zien dat ze samen hebben uitgevoerd');
            $lang_update_queries[] = PT_UpdateLangs($value, 'no_request_found', 'Geen aanvraag gevonden');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tagged_you', 'Tagged u.');
            $lang_update_queries[] = PT_UpdateLangs($value, 'approve', 'Goedkeuren');
            $lang_update_queries[] = PT_UpdateLangs($value, 'decline', 'Afwijzen');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tagged_artists', 'Getagde artiesten');
        } else if ($value == 'french') {
            $lang_update_queries[] = PT_UpdateLangs($value, 'first_name', 'Prénom');
            $lang_update_queries[] = PT_UpdateLangs($value, 'too_many_login_attempts_please_try_again_later', 'Trop de tentatives de connexion, veuillez réessayer plus tard.');
            $lang_update_queries[] = PT_UpdateLangs($value, 'please_choose_which_information_you_would_like_to_download', 'Veuillez choisir les informations que vous souhaitez télécharger.');
            $lang_update_queries[] = PT_UpdateLangs($value, 'my_information', 'Mon information');
            $lang_update_queries[] = PT_UpdateLangs($value, 'generate_file', 'Générer un fichier');
            $lang_update_queries[] = PT_UpdateLangs($value, 'your_file_is_ready_to_download_', 'Votre fichier est prêt à télécharger!');
            $lang_update_queries[] = PT_UpdateLangs($value, 'avatar', 'Avatar');
            $lang_update_queries[] = PT_UpdateLangs($value, 'cover', 'Couverture');
            $lang_update_queries[] = PT_UpdateLangs($value, 'hall_of_fame', 'Temple de la renommée');
            $lang_update_queries[] = PT_UpdateLangs($value, 'enabled', 'Activée');
            $lang_update_queries[] = PT_UpdateLangs($value, 'disabled', 'Désactivée');
            $lang_update_queries[] = PT_UpdateLangs($value, 'donate_button', 'Bouton DONE');
            $lang_update_queries[] = PT_UpdateLangs($value, 'to_enable_donate_button_please_put_your', 'Pour activer le bouton DONATE, veuillez mettre votre');
            $lang_update_queries[] = PT_UpdateLangs($value, 'donate', 'Faire un don');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tag_artists', 'Tag artistes');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tag_other_artists_to_show_they_performed_together', 'Tag d\'autres artistes à montrer qu\'ils ont joué ensemble');
            $lang_update_queries[] = PT_UpdateLangs($value, 'no_request_found', '');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tagged_you', 'tagué tu.');
            $lang_update_queries[] = PT_UpdateLangs($value, 'approve', 'Approuver');
            $lang_update_queries[] = PT_UpdateLangs($value, 'decline', 'Déclin');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tagged_artists', 'Tagged Artists');
        } else if ($value == 'german') {
            $lang_update_queries[] = PT_UpdateLangs($value, 'first_name', 'Vorname');
            $lang_update_queries[] = PT_UpdateLangs($value, 'too_many_login_attempts_please_try_again_later', 'Zu viele Anmeldeversuche versuchen Sie es später erneut.');
            $lang_update_queries[] = PT_UpdateLangs($value, 'please_choose_which_information_you_would_like_to_download', 'Bitte wählen Sie, welche Informationen Sie herunterladen möchten.');
            $lang_update_queries[] = PT_UpdateLangs($value, 'my_information', 'Meine Information');
            $lang_update_queries[] = PT_UpdateLangs($value, 'generate_file', 'Datei generieren');
            $lang_update_queries[] = PT_UpdateLangs($value, 'your_file_is_ready_to_download_', 'Ihre Datei ist bereit zum Herunterladen!');
            $lang_update_queries[] = PT_UpdateLangs($value, 'avatar', 'Benutzerbild');
            $lang_update_queries[] = PT_UpdateLangs($value, 'cover', 'Startseite');
            $lang_update_queries[] = PT_UpdateLangs($value, 'hall_of_fame', 'Ruhmeshalle');
            $lang_update_queries[] = PT_UpdateLangs($value, 'enabled', 'aktiviert');
            $lang_update_queries[] = PT_UpdateLangs($value, 'disabled', 'Deaktiviert');
            $lang_update_queries[] = PT_UpdateLangs($value, 'donate_button', 'Knopf spenden');
            $lang_update_queries[] = PT_UpdateLangs($value, 'to_enable_donate_button_please_put_your', 'Um die Spenden-Schaltfläche zu aktivieren, setzen Sie bitte Ihre');
            $lang_update_queries[] = PT_UpdateLangs($value, 'donate', 'Spenden');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tag_artists', 'Tag Künstler');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tag_other_artists_to_show_they_performed_together', 'TAG ANDERE Künstler, um zu zeigen, dass sie zusammengearbeitet haben');
            $lang_update_queries[] = PT_UpdateLangs($value, 'no_request_found', '');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tagged_you', 'hat dir getaggt.');
            $lang_update_queries[] = PT_UpdateLangs($value, 'approve', '');
            $lang_update_queries[] = PT_UpdateLangs($value, 'decline', 'Ablehnen');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tagged_artists', 'Tagged Künstler');
        } else if ($value == 'russian') {
            $lang_update_queries[] = PT_UpdateLangs($value, 'first_name', 'Имя');
            $lang_update_queries[] = PT_UpdateLangs($value, 'too_many_login_attempts_please_try_again_later', 'Слишком много попыток входа в систему, пожалуйста, попробуйте позже.');
            $lang_update_queries[] = PT_UpdateLangs($value, 'please_choose_which_information_you_would_like_to_download', 'Пожалуйста, выберите, какую информацию вы хотите скачать.');
            $lang_update_queries[] = PT_UpdateLangs($value, 'my_information', 'Моя информация');
            $lang_update_queries[] = PT_UpdateLangs($value, 'generate_file', 'Генерировать файл');
            $lang_update_queries[] = PT_UpdateLangs($value, 'your_file_is_ready_to_download_', 'Ваш файл готов скачать!');
            $lang_update_queries[] = PT_UpdateLangs($value, 'avatar', 'Аватар');
            $lang_update_queries[] = PT_UpdateLangs($value, 'cover', 'Покрытие');
            $lang_update_queries[] = PT_UpdateLangs($value, 'hall_of_fame', 'Зал славы');
            $lang_update_queries[] = PT_UpdateLangs($value, 'enabled', 'Включено');
            $lang_update_queries[] = PT_UpdateLangs($value, 'disabled', 'Отключено');
            $lang_update_queries[] = PT_UpdateLangs($value, 'donate_button', 'Пожертвовать кнопку');
            $lang_update_queries[] = PT_UpdateLangs($value, 'to_enable_donate_button_please_put_your', 'Чтобы включить кнопку пожертвования, пожалуйста, поместите свой');
            $lang_update_queries[] = PT_UpdateLangs($value, 'donate', 'Пожертвовать');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tag_artists', 'Теги художников');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tag_other_artists_to_show_they_performed_together', 'Помещать другие художники, чтобы показать, что они выступали вместе');
            $lang_update_queries[] = PT_UpdateLangs($value, 'no_request_found', 'Ни один запрос не обнаружен');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tagged_you', 'пометили вас.');
            $lang_update_queries[] = PT_UpdateLangs($value, 'approve', 'Утвердить');
            $lang_update_queries[] = PT_UpdateLangs($value, 'decline', 'Отклонить');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tagged_artists', 'Тегистых художников');
        } else if ($value == 'spanish') {
            $lang_update_queries[] = PT_UpdateLangs($value, 'first_name', 'Primer nombre');
            $lang_update_queries[] = PT_UpdateLangs($value, 'too_many_login_attempts_please_try_again_later', 'Demasiados intentos de inicio de sesión, intente nuevamente más tarde.');
            $lang_update_queries[] = PT_UpdateLangs($value, 'please_choose_which_information_you_would_like_to_download', 'Por favor, elija qué información le gustaría descargar.');
            $lang_update_queries[] = PT_UpdateLangs($value, 'my_information', 'Mi informacion');
            $lang_update_queries[] = PT_UpdateLangs($value, 'generate_file', 'Generar archivo');
            $lang_update_queries[] = PT_UpdateLangs($value, 'your_file_is_ready_to_download_', 'Su archivo está listo para descargar!');
            $lang_update_queries[] = PT_UpdateLangs($value, 'avatar', 'Avatar');
            $lang_update_queries[] = PT_UpdateLangs($value, 'cover', 'Cubrir');
            $lang_update_queries[] = PT_UpdateLangs($value, 'hall_of_fame', 'Salón de la Fama');
            $lang_update_queries[] = PT_UpdateLangs($value, 'enabled', 'Activado');
            $lang_update_queries[] = PT_UpdateLangs($value, 'disabled', 'Discapacitado');
            $lang_update_queries[] = PT_UpdateLangs($value, 'donate_button', 'Botón de donar');
            $lang_update_queries[] = PT_UpdateLangs($value, 'to_enable_donate_button_please_put_your', 'Para habilitar el botón Donar, ponga su');
            $lang_update_queries[] = PT_UpdateLangs($value, 'donate', 'Donar');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tag_artists', 'Tag artistas');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tag_other_artists_to_show_they_performed_together', 'Etiquete otros artistas para mostrar que se desempeñaron juntos.');
            $lang_update_queries[] = PT_UpdateLangs($value, 'no_request_found', '');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tagged_you', 'Etiquetado.');
            $lang_update_queries[] = PT_UpdateLangs($value, 'approve', 'Aprobar');
            $lang_update_queries[] = PT_UpdateLangs($value, 'decline', 'Disminución');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tagged_artists', 'Artistas etiquetados');
        } else if ($value == 'turkish') {
            $lang_update_queries[] = PT_UpdateLangs($value, 'first_name', 'İsim');
            $lang_update_queries[] = PT_UpdateLangs($value, 'too_many_login_attempts_please_try_again_later', 'Çok fazla oturum açma girişimi Lütfen daha sonra tekrar deneyin.');
            $lang_update_queries[] = PT_UpdateLangs($value, 'please_choose_which_information_you_would_like_to_download', 'Lütfen hangi bilgileri indirmek istediğinizi seçin.');
            $lang_update_queries[] = PT_UpdateLangs($value, 'my_information', 'Benim bilgim');
            $lang_update_queries[] = PT_UpdateLangs($value, 'generate_file', 'Dosya Oluştur');
            $lang_update_queries[] = PT_UpdateLangs($value, 'your_file_is_ready_to_download_', 'Dosyanız indirmeye hazır!');
            $lang_update_queries[] = PT_UpdateLangs($value, 'avatar', 'Avatar');
            $lang_update_queries[] = PT_UpdateLangs($value, 'cover', 'Örtmek');
            $lang_update_queries[] = PT_UpdateLangs($value, 'hall_of_fame', 'Onur listesi');
            $lang_update_queries[] = PT_UpdateLangs($value, 'enabled', 'Etkin');
            $lang_update_queries[] = PT_UpdateLangs($value, 'disabled', 'Engelli');
            $lang_update_queries[] = PT_UpdateLangs($value, 'donate_button', 'Bağış düğmesi');
            $lang_update_queries[] = PT_UpdateLangs($value, 'to_enable_donate_button_please_put_your', 'Bağış düğmesini etkinleştirmek için lütfen');
            $lang_update_queries[] = PT_UpdateLangs($value, 'donate', 'Bağışlamak');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tag_artists', 'Tag sanatçılar');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tag_other_artists_to_show_they_performed_together', 'Birlikte yaptıklarını göstermek için diğer sanatçıları etiketleyin');
            $lang_update_queries[] = PT_UpdateLangs($value, 'no_request_found', 'İstek bulunamadı');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tagged_you', 'seni etiketledim.');
            $lang_update_queries[] = PT_UpdateLangs($value, 'approve', 'Onaylamak');
            $lang_update_queries[] = PT_UpdateLangs($value, 'decline', 'Düşüş');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tagged_artists', 'Tagged sanatçılar');
        } else if ($value == 'english') {
            $lang_update_queries[] = PT_UpdateLangs($value, 'first_name', 'First Name');
            $lang_update_queries[] = PT_UpdateLangs($value, 'too_many_login_attempts_please_try_again_later', 'Too many login attempts please try again later.');
            $lang_update_queries[] = PT_UpdateLangs($value, 'please_choose_which_information_you_would_like_to_download', 'Please choose which information you would like to download.');
            $lang_update_queries[] = PT_UpdateLangs($value, 'my_information', 'My Information');
            $lang_update_queries[] = PT_UpdateLangs($value, 'generate_file', 'Generate file');
            $lang_update_queries[] = PT_UpdateLangs($value, 'your_file_is_ready_to_download_', 'Your file is ready to download!');
            $lang_update_queries[] = PT_UpdateLangs($value, 'avatar', 'Avatar');
            $lang_update_queries[] = PT_UpdateLangs($value, 'cover', 'Cover');
            $lang_update_queries[] = PT_UpdateLangs($value, 'hall_of_fame', 'Hall of fame');
            $lang_update_queries[] = PT_UpdateLangs($value, 'enabled', 'Enabled');
            $lang_update_queries[] = PT_UpdateLangs($value, 'disabled', 'Disabled');
            $lang_update_queries[] = PT_UpdateLangs($value, 'donate_button', 'Donate Button');
            $lang_update_queries[] = PT_UpdateLangs($value, 'to_enable_donate_button_please_put_your', 'To enable donate button please put your');
            $lang_update_queries[] = PT_UpdateLangs($value, 'donate', 'Donate');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tag_artists', 'Tag Artists');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tag_other_artists_to_show_they_performed_together', 'Tag other artists to show they performed together');
            $lang_update_queries[] = PT_UpdateLangs($value, 'no_request_found', 'No request found');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tagged_you', 'tagged you.');
            $lang_update_queries[] = PT_UpdateLangs($value, 'approve', 'Approve');
            $lang_update_queries[] = PT_UpdateLangs($value, 'decline', 'Decline');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tagged_artists', 'Tagged Artists');
        } else if ($value != 'english') {
            $lang_update_queries[] = PT_UpdateLangs($value, 'first_name', 'First Name');
            $lang_update_queries[] = PT_UpdateLangs($value, 'too_many_login_attempts_please_try_again_later', 'Too many login attempts please try again later.');
            $lang_update_queries[] = PT_UpdateLangs($value, 'please_choose_which_information_you_would_like_to_download', 'Please choose which information you would like to download.');
            $lang_update_queries[] = PT_UpdateLangs($value, 'my_information', 'My Information');
            $lang_update_queries[] = PT_UpdateLangs($value, 'generate_file', 'Generate file');
            $lang_update_queries[] = PT_UpdateLangs($value, 'your_file_is_ready_to_download_', 'Your file is ready to download!');
            $lang_update_queries[] = PT_UpdateLangs($value, 'avatar', 'Avatar');
            $lang_update_queries[] = PT_UpdateLangs($value, 'cover', 'Cover');
            $lang_update_queries[] = PT_UpdateLangs($value, 'hall_of_fame', 'Hall of fame');
            $lang_update_queries[] = PT_UpdateLangs($value, 'enabled', 'Enabled');
            $lang_update_queries[] = PT_UpdateLangs($value, 'disabled', 'Disabled');
            $lang_update_queries[] = PT_UpdateLangs($value, 'donate_button', 'Donate Button');
            $lang_update_queries[] = PT_UpdateLangs($value, 'to_enable_donate_button_please_put_your', 'To enable donate button please put your');
            $lang_update_queries[] = PT_UpdateLangs($value, 'donate', 'Donate');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tag_artists', 'Tag Artists');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tag_other_artists_to_show_they_performed_together', 'Tag other artists to show they performed together');
            $lang_update_queries[] = PT_UpdateLangs($value, 'no_request_found', 'No request found');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tagged_you', 'tagged you.');
            $lang_update_queries[] = PT_UpdateLangs($value, 'approve', 'Approve');
            $lang_update_queries[] = PT_UpdateLangs($value, 'decline', 'Decline');
            $lang_update_queries[] = PT_UpdateLangs($value, 'tagged_artists', 'Tagged Artists');
        }
    }
    if (!empty($lang_update_queries)) {
        foreach ($lang_update_queries as $key => $query) {
            $sql = mysqli_query($mysqli, $query);
        }
        $users = $db->get(T_USERS);
        if (!empty($users)) {
            foreach ($users as $key => $user) {
                if (empty($user->time)) {
                    $array = explode('/', $user->registered);
                    $time = strtotime($array[0].'/'.$array[1].'/01');
                    if ($array[1] == '00' || $array[0] == '0000') {
                        $time = time();
                    }
                    $db->where('id',$user->id)->update(T_USERS,array('time' => $time));
                }
            }
        }
    }
    $name = md5(microtime()) . '_updated.php';
    rename('update.php', $name);
}
?>
<html>
   <head>
      <meta http-equiv="Content-Type" content="text/html; charset=UTF-8"/>
      <meta name="viewport" content="width=device-width, initial-scale=1"/>
      <title>Updating DeepSound</title>
      <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
      <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css">
      <style>
         @import url('https://fonts.googleapis.com/css?family=Roboto:400,500');
         @media print {
            .wo_update_changelog {max-height: none !important; min-height: !important}
            .btn, .hide_print, .setting-well h4 {display:none;}
         }
         * {outline: none !important;}
         body {background: #f3f3f3;font-family: 'Roboto', sans-serif;}
         .light {font-weight: 400;}
         .bold {font-weight: 500;}
         .btn {height: 52px;line-height: 1;font-size: 16px;transition: all 0.3s;border-radius: 2em;font-weight: 500;padding: 0 28px;letter-spacing: .5px;}
         .btn svg {margin-left: 10px;margin-top: -2px;transition: all 0.3s;vertical-align: middle;}
         .btn:hover svg {-webkit-transform: translateX(3px);-moz-transform: translateX(3px);-ms-transform: translateX(3px);-o-transform: translateX(3px);transform: translateX(3px);}
         .btn-main {color: #ffffff;background-color: #f98f1d;border-color: #f98f1d;}
         .btn-main:disabled, .btn-main:focus {color: #fff;}
         .btn-main:hover {color: #ffffff;background-color: #0dcde2;border-color: #0dcde2;box-shadow: -2px 2px 14px rgba(168, 72, 73, 0.35);}
         svg {vertical-align: middle;}
         .main {color: #f98f1d;}
         .wo_update_changelog {
          border: 1px solid #eee;
          padding: 10px !important;
         }
         .content-container {display: -webkit-box; width: 100%;display: -moz-box;display: -ms-flexbox;display: -webkit-flex;display: flex;-webkit-flex-direction: column;flex-direction: column;min-height: 100vh;position: relative;}
         .content-container:before, .content-container:after {-webkit-box-flex: 1;box-flex: 1;-webkit-flex-grow: 1;flex-grow: 1;content: '';display: block;height: 50px;}
         .wo_install_wiz {position: relative;background-color: white;box-shadow: 0 1px 15px 2px rgba(0, 0, 0, 0.1);border-radius: 10px;padding: 20px 30px;border-top: 1px solid rgba(0, 0, 0, 0.04);}
         .wo_install_wiz h2 {margin-top: 10px;margin-bottom: 30px;display: flex;align-items: center;}
         .wo_install_wiz h2 span {margin-left: auto;font-size: 15px;}
         .wo_update_changelog {padding:0;list-style-type: none;margin-bottom: 15px;max-height: 440px;overflow-y: auto; min-height: 440px;}
         .wo_update_changelog li {margin-bottom:7px; max-height: 20px; overflow: hidden;}
         .wo_update_changelog li span {padding: 2px 7px;font-size: 12px;margin-right: 4px;border-radius: 2px;}
         .wo_update_changelog li span.added {background-color: #4CAF50;color: white;}
         .wo_update_changelog li span.changed {background-color: #e62117;color: white;}
         .wo_update_changelog li span.improved {background-color: #9C27B0;color: white;}
         .wo_update_changelog li span.compressed {background-color: #795548;color: white;}
         .wo_update_changelog li span.fixed {background-color: #2196F3;color: white;}
         input.form-control {background-color: #f4f4f4;border: 0;border-radius: 2em;height: 40px;padding: 3px 14px;color: #383838;transition: all 0.2s;}
input.form-control:hover {background-color: #e9e9e9;}
input.form-control:focus {background: #fff;box-shadow: 0 0 0 1.5px #a84849;}
         .empty_state {margin-top: 80px;margin-bottom: 80px;font-weight: 500;color: #6d6d6d;display: block;text-align: center;}
         .checkmark__circle {stroke-dasharray: 166;stroke-dashoffset: 166;stroke-width: 2;stroke-miterlimit: 10;stroke: #7ac142;fill: none;animation: stroke 0.6s cubic-bezier(0.65, 0, 0.45, 1) forwards;}
         .checkmark {width: 80px;height: 80px; border-radius: 50%;display: block;stroke-width: 3;stroke: #fff;stroke-miterlimit: 10;margin: 100px auto 50px;box-shadow: inset 0px 0px 0px #7ac142;animation: fill .4s ease-in-out .4s forwards, scale .3s ease-in-out .9s both;}
         .checkmark__check {transform-origin: 50% 50%;stroke-dasharray: 48;stroke-dashoffset: 48;animation: stroke 0.3s cubic-bezier(0.65, 0, 0.45, 1) 0.8s forwards;}
         @keyframes stroke { 100% {stroke-dashoffset: 0;}}
         @keyframes scale {0%, 100% {transform: none;}  50% {transform: scale3d(1.1, 1.1, 1); }}
         @keyframes fill { 100% {box-shadow: inset 0px 0px 0px 54px #7ac142; }}
      </style>
   </head>
   <body>
      <div class="content-container container">
         <div class="row">
            <div class="col-md-1"></div>
            <div class="col-md-10">
               <div class="wo_install_wiz">
                 <?php if ($updated == false) { ?>
                  <div>
                     <h2 class="light">Update to v1.3.4 </span></h2>
                     <div class="setting-well">
                        <h4>Changelog</h4>
                        <ul class="wo_update_changelog">
                            <li>[Added] ability to tip artists (enable/disable). </li>
                            <li>[Added] hall of fame for artists (new page) (enable/disable). </li>
                            <li>[Added] admin panel notifications. </li>
                            <li>[Added] ability to tag other artists to show they performed together (enable/disable). </li>
                            <li>[Added] new admin panel, v2 with ajax load and faster load. </li>
                            <li>[Added] ability to limit the amount of characters in comments. </li>
                            <li>[Added] support for PHP 8.0+ and MySQL 8.0+</li>
                            <li>[Added] more APIs</li>
                            <li>[Fixed] 15+ reported bugs.</li>
                            <li>[Improved] speed.</li>
                        </ul>
                        <p class="hide_print">Note: The update process might take few minutes.</p>
                        <p class="hide_print">Important: If you got any fail queries, please copy them, open a support ticket and send us the details.</p>
                        <br>
                             <button class="pull-right btn btn-default" onclick="window.print();">Share Log</button>
                             <button type="button" class="btn btn-main" id="button-update">
                             Update 
                             <svg viewBox="0 0 19 14" xmlns="http://www.w3.org/2000/svg" width="18" height="18">
                                <path fill="currentColor" d="M18.6 6.9v-.5l-6-6c-.3-.3-.9-.3-1.2 0-.3.3-.3.9 0 1.2l5 5H1c-.5 0-.9.4-.9.9s.4.8.9.8h14.4l-4 4.1c-.3.3-.3.9 0 1.2.2.2.4.2.6.2.2 0 .4-.1.6-.2l5.2-5.2h.2c.5 0 .8-.4.8-.8 0-.3 0-.5-.2-.7z"></path>
                             </svg>
                          </button>
                     </div>
                     <?php }?>
                     <?php if ($updated == true) { ?>
                      <div>
                        <div class="empty_state">
                           <svg class="checkmark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 52 52">
                              <circle class="checkmark__circle" cx="26" cy="26" r="25" fill="none"/>
                              <path class="checkmark__check" fill="none" d="M14.1 27.2l7.1 7.2 16.7-16.8"/>
                           </svg>
                           <p>Congratulations, you have successfully updated your site. Thanks for choosing DeepSound.</p>
                           <br>
                           <a href="<?php echo $wo['config']['site_url'] ?>" class="btn btn-main" style="line-height:50px;">Home</a>
                        </div>
                     </div>
                     <?php }?>
                  </div>
               </div>
            </div>
            <div class="col-md-1"></div>
         </div>
      </div>
   </body>
</html>
<script>  
var queries = [
    "UPDATE `config` SET value = '1.3.4' WHERE name = 'version';",
    "ALTER TABLE `notifications` ADD `admin` INT(11) NOT NULL DEFAULT '0' AFTER `sent_push`;",
    "ALTER TABLE `notifications` ADD INDEX(`admin`);",
    "ALTER TABLE `users` ADD `time` INT(20) NOT NULL DEFAULT '0' AFTER `email_on_comment_mention`;",
    "ALTER TABLE `users` ADD INDEX(`time`);",
    "INSERT INTO `config` (`id`, `name`, `value`) VALUES (NULL, 'prevent_system', '0');",
    "INSERT INTO `config` (`id`, `name`, `value`) VALUES (NULL, 'bad_login_limit', '4');",
    "INSERT INTO `config` (`id`, `name`, `value`) VALUES (NULL, 'lock_time', '10');",
    "CREATE TABLE `bad_login` (  `id` int(11) NOT NULL AUTO_INCREMENT,  `ip` varchar(100) NOT NULL DEFAULT '',  `time` int(50) NOT NULL DEFAULT 0,  PRIMARY KEY (`id`),  KEY `ip` (`ip`)) ENGINE=InnoDB DEFAULT CHARSET=utf8;",
    "ALTER TABLE `users` ADD `info_file` TEXT CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL AFTER `upload_import`;",
    "INSERT INTO `config` (`id`, `name`, `value`) VALUES (NULL, 'fame_system', '1');",
    "INSERT INTO `config` (`id`, `name`, `value`) VALUES (NULL, 'views_count', '100000');",
    "INSERT INTO `config` (`id`, `name`, `value`) VALUES (NULL, 'donate_system', '1');",
    "ALTER TABLE `users` ADD `paypal_email` VARCHAR(100) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT '' AFTER `upload_import`;",
    "INSERT INTO `config` (`id`, `name`, `value`) VALUES (NULL, 'tag_artist_system', '1');",
    "CREATE TABLE `artists_tags` ( `id` INT(11) NOT NULL AUTO_INCREMENT , `artist_id` INT(11) NOT NULL DEFAULT '0' , `user_id` INT(11) NOT NULL DEFAULT '0' , `track_id` INT(11) NOT NULL DEFAULT '0' , `time` INT(11) NOT NULL DEFAULT '0' , PRIMARY KEY (`id`)) ENGINE = InnoDB;",
    "ALTER TABLE `artists_tags` ADD `approved` INT(11) NOT NULL DEFAULT '0' AFTER `time`, ADD `seen` INT(11) NOT NULL DEFAULT '0' AFTER `approved`;",
    "INSERT INTO `config` (`id`, `name`, `value`) VALUES (NULL, 'maxCharacters', '640');",

];

$('#input_code').bind("paste keyup input propertychange", function(e) {
    if (isPurchaseCode($(this).val())) {
        $('#button-update').removeAttr('disabled');
    } else {
        $('#button-update').attr('disabled', 'true');
    }
});

function isPurchaseCode(str) {
    var patt = new RegExp("(.*)-(.*)-(.*)-(.*)-(.*)");
    var res = patt.test(str);
    if (res) {
        return true;
    }
    return false;
}

$(document).on('click', '#button-update', function(event) {
    if ($('body').attr('data-update') == 'true') {
        window.location.href = '<?php echo $site_url?>';
        return false;
    }
    $(this).attr('disabled', true);
    $('.wo_update_changelog').html('');
    $('.wo_update_changelog').css({
        background: '#1e2321',
        color: '#fff'
    });
    $('.setting-well h4').text('Updating..');
    $(this).attr('disabled', true);
    RunQuery();
});

var queriesLength = queries.length;
var query = queries[0];
var count = 0;
function b64EncodeUnicode(str) {
    // first we use encodeURIComponent to get percent-encoded UTF-8,
    // then we convert the percent encodings into raw bytes which
    // can be fed into btoa.
    return btoa(encodeURIComponent(str).replace(/%([0-9A-F]{2})/g,
        function toSolidBytes(match, p1) {
            return String.fromCharCode('0x' + p1);
    }));
}
function RunQuery() {
    var query = queries[count];
    $.post('?update', {
        query: b64EncodeUnicode(query)
    }, function(data, textStatus, xhr) {
        if (data.status == 200) {
            $('.wo_update_changelog').append('<li><span class="added">SUCCESS</span> ~$ mysql > ' + query + '</li>');
        } else {
            $('.wo_update_changelog').append('<li><span class="changed">FAILED</span> ~$ mysql > ' + query + '</li>');
        }
        count = count + 1;
        if (queriesLength > count) {
            setTimeout(function() {
                RunQuery();
            }, 1500);
        } else {
            $('.wo_update_changelog').append('<li><span class="added">Updating Langauges & Categories</span> ~$ languages.sh, Please wait, this might take some time..</li>');
            $.post('?run_lang', {
                update_langs: 'true'
            }, function(data, textStatus, xhr) {
              $('.wo_update_changelog').append('<li><span class="fixed">Finished!</span> ~$ Congratulations! you have successfully updated your site. Thanks for choosing DeepSound.</li>');
              $('.setting-well h4').text('Update Log');
              $('#button-update').html('Home <svg viewBox="0 0 19 14" xmlns="http://www.w3.org/2000/svg" width="18" height="18"> <path fill="currentColor" d="M18.6 6.9v-.5l-6-6c-.3-.3-.9-.3-1.2 0-.3.3-.3.9 0 1.2l5 5H1c-.5 0-.9.4-.9.9s.4.8.9.8h14.4l-4 4.1c-.3.3-.3.9 0 1.2.2.2.4.2.6.2.2 0 .4-.1.6-.2l5.2-5.2h.2c.5 0 .8-.4.8-.8 0-.3 0-.5-.2-.7z"></path> </svg>');
              $('#button-update').attr('disabled', false);
              $(".wo_update_changelog").scrollTop($(".wo_update_changelog")[0].scrollHeight);
              $('body').attr('data-update', 'true');
            });
        }
        $(".wo_update_changelog").scrollTop($(".wo_update_changelog")[0].scrollHeight);
    });
}
</script>